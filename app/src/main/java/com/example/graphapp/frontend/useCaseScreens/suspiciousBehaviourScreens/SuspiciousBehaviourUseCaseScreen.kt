...
fun SuspiciousBehaviourUseCaseScreen(viewModel: GraphViewModel, navController: NavController) {
    ...
    LaunchedEffect(Unit) {
        eventInputMap[SchemaEventTypeNames.INCIDENT.key] = "Subject lurking around the area, trying to avoid cameras"
        eventInputMap[SchemaEventTypeNames.WHERE.key] = "1.3425,103.6897"
    }
            ...
            Column(
                ...
                testData.forEach { incident ->
                    Card(
                        ...
                        Text(
                            text = "Incident: ${incident[SchemaEventTypeNames.INCIDENT.key]}",
                        ...
                        Text(
                            text = "How: ${incident[SchemaEventTypeNames.HOW.key]}",
                            modifier = Modifier.padding(horizontal = 10.dp).padding(top = 4.dp),
                            style = MaterialTheme.typography.bodyMedium,
                            color = Color.DarkGray,
                        )
                        Text(
                            text = "Observed on: ${incident[SchemaEventTypeNames.WHEN.key]}",
                            modifier = Modifier.padding(vertical = 4.dp, horizontal = 10.dp),
                            style = MaterialTheme.typography.bodyMedium,
                            color = Color.DarkGray,
                        )
                        Text(
                            text = "Location: ${incident[SchemaEventTypeNames.WHERE.key]}",
